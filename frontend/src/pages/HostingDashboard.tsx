...
import { Building2, CheckCircle, XCircle, Clock, LogOut, Upload, Server, Trash2 } from 'lucide-react';
...
export const HostingDashboard: React.FC = () => {
  ...
  const handleClearAllRequests = async () => {
    if (!window.confirm('Are you sure you want to clear ALL requests from view? You can still see them on other dashboards.')) return;
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${API_BASE_URL}/requests/hide-all`, {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${token}`
        }
      ...
      if (res.ok) {
        ...
        alert(data.message || 'Failed to clear requests.');
      }
    } catch (error) {
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200">
                    ...
                    <th className="text-left py-3 px-4 font-medium text-gray-700">Deployment Hash</th>
                    <th className="text-left py-3 px-4 font-medium text-gray-700">Deployed At</th>
                    <th className="text-left py-3 px-4 font-medium text-gray-700">CDT Note</th>
                  ...
                  {requests
                    ...
                    .map((request: Request) => (
                      <tr key={request._id} className="border-b border-gray-100">
                        ...
                        <td className="py-3 px-4 text-gray-600">{request.workflow.cdt?.comment || '—'}</td>
                        <td className="py-3 px-4 text-gray-600">{request.workflow.hosting.comment || '—'}</td>
                      </tr>
                    ))}
                </tbody>
              </table>
