...
const getRequests = async (req, res) => {
  try {
    ...
    // Exclude requests hidden by the current user (except for HostingTeam and Admin)
    if (role !== 'HostingTeam' && role !== 'Admin') {
...
// Clear reviewer history for current user
const clearReviewerHistory = async (req, res) => {
  try {
    const userId = req.user.userId;
    // Find all requests where reviewer.reviewedBy == userId and status is Approved/Rejected
    const requests = await Request.find({ 'workflow.reviewer.reviewedBy': userId });
    for (const reqDoc of requests) {
      if (
        reqDoc.workflow.reviewer &&
        (reqDoc.workflow.reviewer.status === 'Approved' || reqDoc.workflow.reviewer.status === 'Rejected') &&
        String(reqDoc.workflow.reviewer.reviewedBy) === String(userId)
      ) {
        // Remove reviewer history for this user
        reqDoc.workflow.reviewer.status = 'Pending';
        reqDoc.workflow.reviewer.comment = undefined;
        reqDoc.workflow.reviewer.reviewedBy = undefined;
        reqDoc.workflow.reviewer.reviewedAt = undefined;
        // Optionally, remove auditHistory entries for this user and reviewer action
        if (Array.isArray(reqDoc.auditHistory)) {
          reqDoc.auditHistory = reqDoc.auditHistory.filter(
            entry => !(entry.by === req.user.name || entry.by === req.user.email || entry.by === userId) && entry.action === 'approved' || entry.action === 'rejected'
          );
        }
        await reqDoc.save();
      }
    }
    res.json({ message: 'Reviewer history cleared' });
  } catch (error) {
    console.error('Clear reviewer history error:', error);
    res.status(500).json({ message: 'Server error' });
  }
};

// Hide all requests for current user (soft hide, do not delete)
const hideAllRequestsForUser = async (req, res) => {
  try {
    const userId = req.user.userId;
    const { role } = req.user;
    ...
    switch (role) {
      case 'Developer':
        // Hide requests created by this developer
        query = { developer: userId, hiddenBy: { $ne: userId } };
        break;
      case 'Reviewer':
        // Hide requests reviewed by this reviewer
        query = {
          'workflow.reviewer.reviewedBy': userId,
          'workflow.reviewer.status': { $in: ['Approved', 'Rejected'] },
          hiddenBy: { $ne: userId }
        };
        break;
      case 'HOD':
        // Hide requests reviewed by this HOD
        query = {
          'workflow.hod.reviewedBy': userId,
          'workflow.hod.status': { $in: ['Approved', 'Rejected'] },
          hiddenBy: { $ne: userId }
        };
        break;
      case 'DTG':
        // Hide requests reviewed by this DTG
        query = {
          'workflow.dtg.reviewedBy': userId,
          'workflow.dtg.status': { $in: ['Approved', 'Rejected'] },
          hiddenBy: { $ne: userId }
        };
        break;
      case 'CDT':
        // Hide requests reviewed by this CDT
        query = {
          'workflow.cdt.reviewedBy': userId,
          'workflow.cdt.status': { $in: ['Approved', 'Rejected'] },
          hiddenBy: { $ne: userId }
        };
        break;
      case 'HostingTeam':
        // Hide all requests that are hosted or rejected (not just those hosted by this user)
        query = {
          'workflow.hosting.status': { $in: ['Hosted', 'Rejected'] },
          hiddenBy: { $ne: userId }
        };
        break;
      default:
        return res.status(403).json({ message: 'Invalid role for hiding requests' });
    }

    await Request.updateMany(query, { $push: { hiddenBy: userId } });
    res.json({ message: 'All requests hidden for user' });
  } catch (error) {
    console.error('Hide all requests error:', error);
    res.status(500).json({ message: 'Server error' });
  }
};
